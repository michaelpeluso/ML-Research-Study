...
from agents import SARSA, set_seeds, get_git_sha, make_filename
...
def main():
    parser = argparse.ArgumentParser(description='sarsa on blackjack')
    ...
    print(f"training sarsa on blackjack for {args.episodes} episodes...")
    agent = SARSA(env, alpha=args.alpha, gamma=args.gamma, epsilon=args.epsilon, 
    ...
    ax1.set_title('sarsa learning curve (blackjack)')
    ax1.legend()
    ax1.grid(alpha=0.3)
    ...
    filename = make_filename('blackjack_sarsa', sha, 'png', timestamp, 'learning_curve')
