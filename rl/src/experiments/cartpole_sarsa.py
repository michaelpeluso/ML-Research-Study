# ai use statement: github copilot assisted with discretization and sarsa cartpole integration
"""sarsa learning on cartpole with discretization."""
...
from agents import SARSA, StateDiscretizer, set_seeds, get_git_sha, make_filename
...
class DiscreteSARSA(SARSA):
    """sarsa with state discretization for continuous spaces."""
...
def main():
    ...
    agent = DiscreteSARSA(env, discretizer, alpha=args.alpha, gamma=args.gamma, 
    ...
    ax1.plot(range(window-1, len(rewards)), smoothed, color='darkblue', linewidth=2, label=f'{window}-episode avg')
    ax1.set_xlabel('episode')
    ax1.set_ylabel('episode length')
    ax1.set_title('sarsa learning curve (cartpole)')
    ax1.legend()
    ax1.grid(alpha=0.3)
    ...
    filename = make_filename('cartpole_sarsa', sha, 'png', timestamp, 'learning_curve')
