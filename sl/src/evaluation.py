...
def evaluate_test_set(pipeline, X_test, y_test):
    ...
    if isinstance(estimator, ClassifierMixin):
        ...
        if y_proba is not None:
            ...
            metrics['average_precision'] = average_precision_score(y_test, y_proba[:, 1] if n_classes == 2 else y_proba, average=average_type if n_classes > 2 else None)
            metrics['roc_auc'] = roc_auc_score(y_test, y_proba[:, 1] if n_classes == 2 else y_proba, multi_class='ovr' if n_classes > 2 else 'raise', average=average_type if n_classes > 2 else None)
...
def compute_learning_curve(pipeline, X_train, y_train, scoring, seed):
    ...
    method = pipeline.named_steps['estimator']._estimator_type
...
def compute_complexity_curve(pipeline, X_train, y_train, param_name, param_values=None, n_splits=5, seed=1):
    if param_values is None:
        param_defaults = {
            "estimator__max_depth": [3, 6, 10, 15, 20],  # For DT
            "estimator__ccp_alpha": [0.0, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2],  # For DT pruning
            "estimator__alpha": [1e-5, 5e-5, 1e-4, 5e-4, 1e-3],  # For LSVM
            "estimator__C": [0.1, 0.5, 1, 2, 10],  # For SVM
            "estimator__n_neighbors": [1, 3, 5, 11, 21, 31],  # For kNN
            "estimator__learning_rate_init": [0.0001, 0.001, 0.01, 0.1]  # For NN
        }
