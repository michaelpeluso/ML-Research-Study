...
def tune_parameter_grid(dataset, pipeline, X, y, param_grid, cv_splits=5, combination_cap=50, seed=1):
    ...
    search.fit(X, y)
    save_results(search, dataset)

    return pipeline, search.best_params_
...
def save_results(search, dataset):
    if hasattr(search, 'cv_results_'):
        results_df = pd.DataFrame(search.cv_results_)
    else:
        results_df = pd.DataFrame(columns=['mean_test_score', 'params'])
    os.makedirs(f"figures/{dataset}", exist_ok=True)
    file_path = f"figures/{dataset}/tuning_results_{dataset}_{search.estimator.__class__.__name__.lower()}.csv"
