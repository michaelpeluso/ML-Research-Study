...
def train_nn_on_new_inputs(
    ...
    for seed in seeds:
        set_seed(seed)
        model = MLP(in_dim=in_dim, hidden=hidden_layers, out_dim=out_dim, dropout_p=0.0).to(self.device)  # No dropout (baseline best)
        ...
        curves, _, wall_time, final_train_loss = train_to_budget(
            ...
            max_updates, float('inf'), loss_fn, self.device, eval_interval=eval_interval # type: ignore
        )

        test_mse = eval_loss(model, new_X_test, loss_fn, self.device)
