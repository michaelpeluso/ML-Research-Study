...
class BaseDR(ABC):
    ...
    def run_dimensionality_reduction(self, X_train, y_train, n_components_range, task='classification'):
        ...
        # Compile metric arrays across all n_components for plotting/analysis
        reconstruction_errors = [r['reconstruction_error'] for r in results if r['reconstruction_error'] is not None]
        explained_variances = [r['explained_variance'] for r in results if r['explained_variance'] is not None]
        cumulative_variances = [r['cumulative_variance'] for r in results if r['cumulative_variance'] is not None]
        ...
        result = {
            ...
            'total_time': elapsed,
            # Add arrays for easy plotting/CSV generation
            'reconstruction_errors': reconstruction_errors if reconstruction_errors else None,
            'explained_variances': explained_variances if explained_variances else None,
            'cumulative_variances': cumulative_variances if cumulative_variances else None,
            'n_components_tested': [r['n_components'] for r in results]
        }
        
        # Add algorithm-specific arrays
        if self.algorithm_name == 'PCA' and best_result['model'] and hasattr(best_result['model'], 'explained_variance_ratio_'):
            result['explained_variance_ratio'] = best_result['model'].explained_variance_ratio_
        
        if self.algorithm_name == 'ICA' and best_result['model'] and hasattr(best_result['model'], 'mixing_'):
            # Compute kurtosis for all components
            from scipy.stats import kurtosis
            mixing = best_result['model'].mixing_
            result['kurtosis'] = kurtosis(mixing, axis=0)
