...
def run_neural_networks_with_clusters(
    ...
    **kwargs
    ...
    X can be either original data or DR-transformed data, depending on use case.
    Clustering results should match the space of X.
    """
    ...
    if em_model is not None and hasattr(em_model, 'predict_proba'):
        try:
            em_responsibilities = em_model.predict_proba(X)
    ...
    # Create baseline comparison (input features only - could be original or DR-transformed)
    print("\n" + "-"*80 + "\nBaseline: NN on Input Features (for comparison)\n" + "-"*80)
    results['baseline'] = train_nn_on_features(
        X, y, "Input Features", method, seed, device,
    ...
    kmeans_cluster_features = extract_cluster_features(
        X, kmeans_labels, cluster_centers=kmeans_centers, feature_mode='all'
    )
    ...
    if feature_setup == 'replacement':
        ...
        X_kmeans = np.concatenate([X, kmeans_cluster_features], axis=1)
        feature_desc = "Input + K-Means Cluster Features"
    
    print(f"K-Means feature shape: {X_kmeans.shape} (input: {X.shape})")
    ...
    results['kmeans'] = train_nn_on_features(
        X_kmeans, y, feature_desc, method, seed, device,
    ...
    if tune_if_needed and results['kmeans']['final_train_loss'] > results['baseline']['final_train_loss'] * 1.5:
        ...
        results['kmeans_tuned'] = train_nn_on_features(
            X_kmeans, y, f"{feature_desc} (Tuned)", method, seed, device,
            batch_size, kmeans_max_updates, kmeans_lr, weight_decay, hidden_layers, activation, ml_logger,
            n_cluster_features=kmeans_cluster_features.shape[1]
        )
    ...
    em_cluster_features = extract_cluster_features(
        X, em_labels, cluster_centers=em_means, 
    ...
    if feature_setup == 'replacement':
        ...
        X_em = np.concatenate([X, em_cluster_features], axis=1)
        feature_desc = "Input + EM/GMM Cluster Features"
    
    print(f"EM/GMM feature shape: {X_em.shape} (input: {X.shape})")
    ...
    results['em'] = train_nn_on_features(
        X_em, y, feature_desc, method, seed, device,
    ...
    if tune_if_needed and results['em']['final_train_loss'] > results['baseline']['final_train_loss'] * 1.5:
        ...
        results['em_tuned'] = train_nn_on_features(
            X_em, y, f"{feature_desc} (Tuned)", method, seed, device,
            batch_size, em_max_updates, em_lr, weight_decay, hidden_layers, activation, ml_logger,
            n_cluster_features=em_cluster_features.shape[1]
        )
...
def train_nn_on_features(
    ...
    curves, steps_to_threshold, wall_time, final_train_loss = train_to_budget(
        ...
        log_interval=100, eval_interval=25  # Changed from 100 to 1 for max_updates=5
    )
