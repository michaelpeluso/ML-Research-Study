...
def run_neural_networks(
    ...
    dr_results: Optional[Dict[str, Any]] = None,
    cache_key: str = 'neural_networks',
    step_name: str = "Training Neural Networks"
) -> Dict[str, Any]:
    """Train neural networks on any combination of data (original, DR-transformed, or both)."""
    cache_manager = CacheManager()
    
    # Build cache params
    dr_methods = sorted(list(dr_results.keys())) if dr_results else []
    cache_params = {
        ...
        'dropout_p': dropout_p, 'l_threshold': l_threshold,
        'label_smoothing_alpha': label_smoothing_alpha, 'activation': activation,
        'n_classes': len(np.unique(y_original)), 'dr_methods': dr_methods,
        'has_original': X_original is not None
    }
    
    # Check cache
    cached_result = cache_manager.load(dataset, cache_key, cache_params)
    if cached_result is not None:
        print(f"Loaded cached results for {dataset} {cache_key}")
        return cached_result
    else:
        print(f"No cached results found for {dataset} {cache_key}, training from scratch")
    ...
    # Performance comparison (if multiple configs)
    if len(X_data_dict) > 1:
        _print_performance_comparison(results, [k for k in X_data_dict.keys() if k != 'original'])
    ...
    _log_step_results(ml_logger, step_name, results, hyperparameters, dataset)
    
    # Cache
    cache_manager.save(dataset, cache_key, cache_params, results)
    print(f"Cached {cache_key} results")
    return results
...
def run_neural_networks_on_data(
    ...
    Trains neural networks on original data and all DR-transformed versions (PCA, ICA, RP).
    """
    print("\n" + "="*80)
    ...
    # Train on both original and DR data in one call
    results = run_neural_networks(
        ...
        cache_key='neural_networks_on_data',
        step_name="STEP 4: Training Neural Networks on Original and Reduced Data"
    )
