...
TUNING_GRIDS = {
    "dt": {
        "classification": {
            "max_depth": [6, 10, 14, 18, 22, 28, None], 
    ...
    "knn": {
        "classification": {
            "n_neighbors": [3, 5, 7, 9, 11, 13, 15, 18, 21],
        },
        "regression": {
...
PRIMARY_PARAMS = {
    ...
    "svm": "C",
    "knn": "n_neighbors",
    "nn": "learning_rate_init"
}


def get_tuning_grid(method: str, model: str):
    method_key = "classification" if method.lower().startswith("c") else "regression"
    hyperparams = TUNING_GRIDS.get(model, {}).get(method_key, {})
    return prefix_params(hyperparams)

def get_primary_params(model: str):
    primary_param = PRIMARY_PARAMS.get(model)
    return f"estimator__{primary_param}"
...
def prefix_params(hyperparams):
    prefixed = {}
    for k, v in hyperparams.items():
        if not k.startswith("estimator__"):
            prefixed[f"estimator__{k}"] = v
        else:
            prefixed[k] = v
    return prefixed