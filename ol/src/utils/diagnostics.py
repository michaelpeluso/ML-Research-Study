...
def get_model_info(pipeline):
    ...
    if model_type in ["DecisionTreeClassifier", "DecisionTreeRegressor"]:
        ...
        info.update({
            "loss": getattr(model, "loss", "N/A"),  # Corrected: not "kernel"
        ...
        if hasattr(model, "probability"):
            info["probability_enabled"] = model.probability
        ...
        if hasattr(model, "support_vectors_") and hasattr(model, "n_support_"):
            ...
            info["support_vector_fraction"] = (str)(total_samples / len(model.support_vectors_)) if total_samples != "N/A" else "N/A"
        if hasattr(model, "probability"):
            info["probability_enabled"] = model.probability
    elif model_type in ["MLPClassifier", "MLPRegressor"]:
        info.update({
            "hidden_layer_sizes": getattr(model, "hidden_layer_sizes", "N/A"),
        ...
        if hasattr(model, "n_outputs_"):
            info["n_outputs"] = getattr(model, "n_outputs_", "N/A")
        ...
        if hasattr(model, "coefs_"):
            ...
            info["n_params"] = (str)(param_count)
    else:
        info["additional_info"] = "No specific info available for this model."
