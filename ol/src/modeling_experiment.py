...
class ModelingExperiment:
    ...
    def configure_pipeline(self, X_train):
        with self.ml_logger.log_step("Pipeline Building") as step_info:
            ...
            pipeline = build_pipeline(transformer, self.model, self.method, self.seed, **self.best_params)
    ...
    # train for best hyperparameters
    def train_estimator(self, pipeline, X_train, y_train):
        with self.ml_logger.log_step("Hyperparameter Tuning") as step_info:
            param_grid = get_tuning_grid(self.method, self.model)

            if self.model == "nn" and "estimator__hidden_layer_sizes" in param_grid:
                self.save_path = f"figures/{self.dataset}/{self.model}_{len(param_grid['estimator__hidden_layer_sizes'])}"
                os.makedirs(self.save_path, exist_ok=True)

            if param_grid: 
                pipeline, search = tune_parameter_grid(self.dataset, pipeline, X_train, y_train, param_grid, self.cv_splits, self.combination_cap, self.seed)                
                if hasattr(search, 'cv_results_'): save_results(search, self.dataset, self.save_path)
        ...
        step_info.update({'parameter_count': len(self.best_params), 'best_params': self.best_params})

    # capture execution metrics
    def runtime_measurement(self, pipeline, X_train, y_train, X_test):
        with self.ml_logger.log_step("Runtime Measurement") as step_info:
            ...
            step_info.update({**runtime_data, **hardware_info, **model_info, "best_params": self.best_params, "train_samples": len(X_train), "test_samples": len(X_test), "n_features": X_train.shape[1]})

    # 5. model complexity data
    def model_complexity_data(self, pipeline, X_train, y_train):  # Fixed typo
    ...
    def compute_learning_data(self, pipeline, X_train, y_train):
        ...
        return scoring, train_sizes, train_scores, val_scores

    def model_specific_tasks(self, pipeline, X_train, y_train): 
        if self.model == "dt":  # Decision Tree Pruning path
        ...
        if self.model == "nn":  # NN epoch curve
    ...
    def evaluate_on_test_set(self, pipeline, X_test, y_test):
    ...
    # 10. Generate plots
    def generate_plots(self, pipeline, X_train, X_test, y_test, y_pred, metrics, train_sizes, train_scores, val_scores, scoring):
    ...
    def run(self):
        ...
        pipeline = self.configure_pipeline(X_train)
        
        if self.tuning or self.best_params is None:
            self.train_estimator(pipeline, X_train, y_train)

        # Curves & specifics (multi-fits for diagnostics)
        self.model_complexity_data(pipeline, X_train, y_train)
        scoring, train_sizes, train_scores, val_scores = self.compute_learning_data(pipeline, X_train, y_train)           
        self.model_specific_tasks(pipeline, X_train, y_train)

        # Final single fit
        print("Final refit with best parameters for evaluation and runtime.")
        pipeline.fit(X_train, y_train)

        # Runtime
        self.runtime_measurement(pipeline, X_train, y_train, X_test)
        metrics, y_pred = self.evaluate_on_test_set(pipeline, X_test, y_test)

        self.generate_plots(pipeline, X_train, X_test, y_test, y_pred, metrics, train_sizes, train_scores, val_scores, scoring)
